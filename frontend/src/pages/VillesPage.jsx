...
import AddVille from "../components/admin/AddVilleModal";
...
export default function VillesPage() {
  ...
  const filteredData = useMemo(() => {
    return villes.filter((v) => v.nom?.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [searchTerm, villes]);
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
      ...
      {/* EN-TÊTE — allégé : font-bold au lieu de font-black, plus d'ombre colorée */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <Building2 size={20} className="text-orange-600" /> Villes
          </h1>
          <p className="text-slate-400 text-sm mt-0.5">Administration des centres urbains</p>
        </div>
      ...
      {/* RECHERCHE & OPTIONS — bordure fine, ombre discrète */}
      ...
      {/* AJOUT : liste en cartes sur mobile, plus agréable qu'un tableau à
          faire défiler horizontalement sur un petit écran. */}
      <div className="md:hidden space-y-2 mb-4">
        {currentItems.length === 0 ? (
          <p className="text-center text-slate-400 text-sm py-10">Aucune ville trouvée.</p>
        ) : (
          currentItems.map((item) => (
            <div key={item._id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-lg bg-orange-50 flex items-center justify-center text-orange-600 shrink-0">
                  <Map size={16} />
                </div>
                <span className="text-sm font-semibold text-slate-700 truncate">{item.nom}</span>
              </div>
              <RowActions
                onDelete={() => { setIdToDelete(item._id); setShowModalConfirm(true); }}
              />
            </div>
          ))
        )}
      ...
      {/* TABLEAU — visible à partir de md, radius et ombre allégés */}
      <div className="hidden md:block bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Nom de la ville</th>
                <th className="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {currentItems.map((item, idx) => (
                <tr key={item._id || idx} className="hover:bg-slate-50/70 transition-colors group">
        ...
        <Pagination
          ...
          label={`${currentItems.length} sur ${filteredData.length} villes`}
        />
      </div>

      {/* Pagination mobile, séparée du tableau (masqué en dessous de md) */}
...
// AJOUT : "Détails"/"Modifier" n'étaient reliés à rien dans la version
// d'origine (aucun onClick). Plutôt que de laisser croire qu'ils
// fonctionnent, ils sont désactivés avec une infobulle honnête — à câbler
// dans un prochain passage si besoin.
