...
import { 
  Building2, MapPin, Banknote, Coffee, Monitor,Wind,Wifi, X, Flame, 
  Image as ImageIcon, CheckCircle2, AlertCircle, 
  Loader2, ChevronDown, Zap, Droplet, Pencil, Home, Sofa
} from "lucide-react";
...
export default function ModifAppartementModal({ isOpen, onClose, onSuccess, appartement }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // EXCLUSION DES IMAGES ET DES METADONNÉES MONGO
      ...
      <div className="bg-white text-gray-900 rounded-[2.5rem] w-full max-w-4xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col border border-gray-200">
        ...
        <div className="px-8 py-6 border-b-2 border-gray-100 flex justify-between items-center bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-gray-900 tracking-tight">Modifier l'appartement</h2>
              <p className="text-gray-500 font-bold text-sm">Réf: #{appartement._id.slice(-6).toUpperCase()}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-orange-600 mb-4">01. Localisation & Statut</h3>
              ...
              {/* <InputBlock label="Nom / Titre de l'annonce" icon={Home} value={form.titre} onChange={(v) => handleChange("titre", v)} /> */}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-black text-gray-900 ml-1">Prix & Type de contrat</label>
                <div className="flex items-stretch border-2 border-gray-300 rounded-2xl overflow-hidden focus-within:border-orange-600 transition-all shadow-sm">
                  ...
                  <div className="relative flex-1 bg-white flex items-center">
                    ...
                    <input type="number" className="w-full pl-3 pr-4 py-4 text-lg font-black text-gray-900 outline-none" value={form.prix} onChange={(e) => handleChange("prix", e.target.value)} />
                  </div>
                </div>
              </div>
                <div className="grid grid-cols-2 gap-4">
            ...
            {/* COLONNE DROITE */}
            <div className="space-y-6">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-orange-600 mb-4">02. Composition du bien</h3>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-black text-gray-900 ml-1">Description détaillée</label>
                <textarea 
                  className="w-full bg-white border-2 border-gray-300 rounded-2xl p-4 text-gray-900 font-bold focus:border-orange-600 outline-none h-[115px] transition-all shadow-sm resize-none"
          ...
          <div className="pt-6 border-t-2 border-gray-100">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 mb-6">03. Services & Énergie</h3>
          ...
          {/* VISUEL PHOTOS (LECTURE SEULE) */}
          <div className="bg-gray-50 p-6 rounded-[2rem] border-2 border-gray-100">
            <h3 className="text-xs font-black text-gray-400 uppercase mb-4 flex items-center gap-2 justify-center">
              <ImageIcon size={14}/> Photos de l'appartement
            </h3>
            <div className="flex flex-wrap justify-center gap-4">
              {form.images?.map((src, i) => (
                <div key={i} className="relative w-24 h-24 rounded-2xl overflow-hidden shadow-sm border-2 border-white">
                  <img src={src} className="w-full h-full object-cover opacity-70" alt="appartement" />
                </div>
              ))}
            </div>
          </div>
          ...
          {error && (
            <div className="p-4 bg-red-600 text-white rounded-2xl flex items-center gap-3 font-black text-sm">
...
function InputBlock({ label, icon: Icon, value, onChange }) {
  return (
    <div className="flex flex-col gap-2">
  ...
  return (
    <div className="flex flex-col gap-2 flex-1">
      ...
      <input 
        ...
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-gray-50 border-2 border-gray-300 rounded-2xl py-4 text-center font-black text-gray-900 text-xl focus:border-orange-600 outline-none"
      />
    </div>
  );
}
