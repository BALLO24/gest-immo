...
export default function AddAppartementModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    fd.append("prixParJour",form.prixParJour);
    fd.append("prixParHeure",form.prixParHeure)
      ...
      <div className="bg-gray-900 text-white rounded-2xl w-full max-w-3xl p-6 relative shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit}>
          ...
          <FloatingInput
            ...
            onChange={(e) => handleChange("prixParJour", e.target.value)}
            icon={DollarSign}
          />

          <FloatingInput
            label="Prix (XOF) / Heure"
            type="number"
            value={form.prixParHeure}
          ...
          <div className="col-span-2 grid grid-cols-2 md:grid-cols-3 gap-2 mt-2">
            ...
            <Checkbox label="Meublé" checked={form.meuble} onChange={(v) => handleChange("meuble", v)} icon={Home} />
            <Checkbox label="Climatisation" checked={form.climatisation} onChange={(v) => handleChange("climatisation", v)} icon={Car} />
            <Checkbox label="Balcon" checked={form.balcon} onChange={(v) => handleChange("balcon", v)} icon={Car} />
            <Checkbox label="Internet" checked={form.connexionInternet} onChange={(v) => handleChange("connexionInternet", v)} icon={Car} />
            <Checkbox label="Energie de secours" checked={form.energieSecours} onChange={(v) => handleChange("energieSecours", v)} icon={Car} />
          ...
          <button type="submit" disabled={submitting} className="col-span-2 mt-4 px-6 py-2 bg-maliOrange hover:bg-maliOrange/90 rounded-xl font-semibold">
