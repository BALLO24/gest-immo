...
export default function ModifMagasinModal({ isOpen, onClose, onSuccess, magasin }) {
  ...
  useEffect(() => {
    if (isOpen && magasin) {
      setForm({
        ...magasin,
        agence: magasin.agence?._id || magasin.agence,
        quartier: magasin.quartier?._id || magasin.quartier,
        statut: magasin.statut === "nonDisponible" ? "nonDisponible" : "disponible"
      });
      setVilleSelected(magasin.quartier?.ville?._id || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await API.updateHabitation(magasin._id, finalPayload);
      ...
      <div className="bg-white text-gray-900 rounded-2xl w-full max-w-4xl max-h-[95vh] overflow-hidden shadow-xl flex flex-col border border-gray-200">
        ...
        <div className="px-8 py-6 border-b-2 border-gray-100 flex justify-between items-center bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Modifier le magasin</h2>
              <p className="text-gray-500 font-bold text-sm">Réf: #{magasin._id.slice(-6).toUpperCase()}</p>
            </div>
          </div>
          <button onClick={onClose} aria-label="Fermer" className="p-3 hover:bg-gray-100 rounded-full transition-all text-gray-400 hover:text-black">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-orange-600 mb-4">01. Informations Globales</h3>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-gray-900 ml-1">Prix & Transaction</label>
                <div className="flex items-stretch border border-gray-200 rounded-2xl overflow-hidden focus-within:border-orange-600 transition-all shadow-sm">
                  <select 
                    value={form.typeOffre} 
                    onChange={(e) => handleChange("typeOffre", e.target.value)}
                    className="bg-gray-50 border-r-2 border-gray-300 px-4 text-sm text-gray-900 outline-none cursor-pointer"
                  >
                  ...
                  <div className="relative flex-1 bg-white flex items-center">
                    ...
                    <input 
                      type="number"
                      className="w-full pl-3 pr-4 py-4 text-base text-gray-900 outline-none"
                      value={form.prix || ""}
                      onChange={(e) => handleChange("prix", e.target.value)}
                    />
                    <span className="pr-4 text-xs font-bold text-gray-400">FCFA</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* COLONNE DROITE */}
            <div className="space-y-6">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-orange-600 mb-4">02. Détails du Bien</h3>
              <div className="grid grid-cols-3 gap-3">
              <NumberBox label="Toilettes interne" value={form.toiletteInterne} onChange={(v) => handleChange("toiletteInterne", v)} />

              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-gray-900 ml-1">Description libre</label>
                <textarea 
          ...
          <div className="pt-6 border-t-2 border-gray-100">
            <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-gray-400 mb-6">03. Commodités incluses</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
              ...
              <OptionCard label="🔥 HOT" checked={form.misEnAvant} icon={Flame} onChange={(v) => handleChange("misEnAvant", v)} isHot />
            </div>
          </div>

          {/* PHOTOS (LECTURE SEULE) */}
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
            <h3 className="text-xs font-bold text-gray-400 uppercase mb-4">Photos actuelles</h3>
          ...
          {error && <div className="p-4 bg-red-600 text-white rounded-2xl flex items-center gap-3 font-bold text-sm"><AlertCircle size={20} /> {error}</div>}

          <div className="flex items-center gap-4 pt-4 pb-2">
            <button type="button" onClick={onClose} className="px-8 py-5 text-gray-500 font-bold">ANNULER</button>
            <button 
              type="submit" 
              disabled={submitting}
              className="flex-1 py-5 bg-gray-900 hover:bg-blue-600 text-white rounded-2xl font-bold text-lg transition-all flex items-center justify-center gap-3"
            >
...
function NumberBox({ label, value, onChange }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-[10px] font-bold text-gray-500 text-center uppercase">{label}</label>
      <input type="number" value={value || 0} onChange={(e) => onChange(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-2xl py-4 text-center text-gray-900 text-lg focus:border-orange-600 outline-none" />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2 flex-1">
      ...
      <div className="relative">
        <select value={value} onChange={(e) => onChange(e.target.value)} disabled={disabled} className="w-full appearance-none bg-white border border-gray-200 rounded-2xl px-4 py-3 text-gray-900 text-sm focus:border-orange-600 outline-none disabled:bg-gray-50 cursor-pointer transition-colors">
          {children}
        </select>
