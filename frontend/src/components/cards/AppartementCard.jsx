...
import toast from "react-hot-toast"; // CORRIGÉ : Toaster retiré d'ici, un seul rendu globalement dans App.jsx
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm sm:text-xl font-bold text-maliOrange whitespace-nowrap" aria-label="Prix">
              {/* CORRIGÉ (bug) : sur la page vente, typePaiementAppart n'est
                  plus transmis (retombe sur "journalier" par défaut) — un
                  appartement EN VENTE affichait alors son prixParJour, qui
                  vaut toujours null pour une vente (garde-fou du modèle).
                  On vérifie maintenant typeOffre en priorité : en vente, le
                  prix s'affiche seul, sans période ni dépendance à ce prop. */}
              {appartement.typeOffre === "vente"
                ? `${appartement?.prix?.toLocaleString()} XOF`
                : <>
            ...
            <div className="flex gap-2">
              ...
              <Link
                to={`/bien/${appartement.slug || appartement._id}`}
