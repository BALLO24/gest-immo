      ...
      >
        ...
        <div className="relative w-full h-56 overflow-hidden" role="region" aria-label={`Galerie photos du magasin ${idCourt}`}>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1" role="tablist" aria-label="Navigation par points">
            {magasin.images.map((_, i) => (
              ...
              >
                <span className={`w-2 h-2 rounded-full ${i === currentImage ? "bg-maliOrange" : "bg-white/70 hover:bg-maliOrange/60"} transition-all duration-300`} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center mt-auto">
            <span className="text-sm sm:text-xl font-bold text-maliOrange whitespace-nowrap" aria-label={magasin.typeOffre === "location" ? "Loyer mensuel" : "Prix de vente"}>
              {/* CORRIGÉ : "/ mois" s'affichait même pour un magasin en vente */}
              {magasin?.prix?.toLocaleString() || "0"} XOF{magasin.typeOffre === "location" ? " / mois" : ""}
            </span>
            ...
            <div className="flex gap-2">
              ...
              <Link
                to={`/bien/${magasin.slug || magasin._id}`}
