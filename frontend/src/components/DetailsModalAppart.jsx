...
import { MapPin, Map, Locate, Sofa, Bed, Bath, Square, ChefHat, Store, Snowflake , Droplets, Zap, Wallet, MessageCircle,Phone, X } from "lucide-react";
...
export default function DetailsModalAppart({ item, onClose }) {
  ...
  const infos = [
    ...
    { Icon: Zap, label: "Groupe ou panneau", value: item.energieSecours, status: item.energieSecours },
  ];
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="flex-1 bg-gray-50 rounded-lg border border-gray-200 p-2 flex items-center justify-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-md font-bold text-gray-500">{item.prix ? item.prix.toLocaleString() +" XOF /mois" : "Non défini"}</span> |
                    <span className="text-md font-bold text-gray-500">{item.prixParJour ? item.prixParJour.toLocaleString() +" XOF /jour" : "Non défini"}</span> |
